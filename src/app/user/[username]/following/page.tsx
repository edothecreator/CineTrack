...
export async function generateMetadata({ params }: PageProps) {
  ...
  return { title: `@${username} following — CineTrack` };
}
...
export default async function FollowingPage({ params }: PageProps) {
  ...
  const { users } = await getFollowing(target.id, viewerUserId);
  ...
  return (
    <div className="container max-w-2xl py-10">
      ...
      <h1 className="mb-6 text-2xl font-black tracking-tight">Following</h1>
      {users.length === 0 ? (
